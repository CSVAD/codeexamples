...
void setupGUI(){
  ...
  sliders[si++] = controlP5.addSlider("sd",60,200,left,top+posY+0,len,15);
  posY += 30;

  sliders[si++] = controlP5.addSlider("seedCount",10,200,left,top+posY+0,len,15);
  sliders[si++] = controlP5.addSlider("velocityScaling",0.5,5.0,left,top+posY+20,len,15);
...
void controlEvent(ControlEvent theEvent) {
  ...
  if(theEvent.isController()) {
    if (theEvent.getController().getId() == 3) {
      for(int i=0; i<seedSystems.size(); i++) seedSystems.get(i).updateParams();
    }
  }
}
